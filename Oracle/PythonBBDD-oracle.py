...
def listar_usuarios_con_facturas():
    ...
    if conexion:
        try:
            ...
            consulta = """
            SELECT nombre, usuario.DNI, COUNT(*) AS cantidad_facturas 
            FROM USUARIO 
            INNER JOIN FACTURA ON usuario.DNI = factura.DNI 
            GROUP BY nombre, usuario.DNI
            """
            cursor.execute(consulta)
            resultados = cursor.fetchall()
            for nombre, dni, cantidad_facturas in resultados:
                print(f"Nombre: {nombre} | DNI: {dni} | Facturas: {cantidad_facturas}")
            cursor.close()
            conexion.close()
        except cx_Oracle.Error as error:
...
def insertar_nueva_factura():
    ...
    if conexion:
        try:
            ...
            consulta_insertar_factura = "INSERT INTO FACTURA (id_factura, DNI, fecha) VALUES (:id_factura, :dni, TO_DATE(:fecha, 'YYYY-MM-DD'))"
            cursor.execute(consulta_insertar_factura, id_factura=id_factura, dni=dni_usuario, fecha=fecha)
            conexion.commit()
...
def borrar_facturas_por_dni():
    ...
    if conexion:
        try:
            ...
            consulta_borrar_informes = "DELETE FROM informe_tecnico2 WHERE cod_linea IN (SELECT cod_linea FROM linea_de_factura WHERE id_factura IN (SELECT id_factura FROM factura WHERE DNI = :dni))"
            cursor.execute(consulta_borrar_informes, dni=dni_usuario)

            # Borrar los libros editados asociados a las líneas de factura del usuario
            consulta_borrar_libros = "DELETE FROM libro_editado WHERE cod_linea IN (SELECT cod_linea FROM linea_de_factura WHERE id_factura IN (SELECT id_factura FROM factura WHERE DNI = :dni))"
            cursor.execute(consulta_borrar_libros, dni=dni_usuario)

            # Borrar las líneas de factura asociadas a las facturas del usuario
            consulta_borrar_lineas = "DELETE FROM linea_de_factura WHERE id_factura IN (SELECT id_factura FROM factura WHERE DNI = :dni)"
            cursor.execute(consulta_borrar_lineas, dni=dni_usuario)
            num_lineas_borradas = cursor.rowcount

            # Borrar las facturas del usuario
            consulta_borrar_facturas = "DELETE FROM factura WHERE DNI = :dni"
            cursor.execute(consulta_borrar_facturas, dni=dni_usuario)
...
def actualizar_usuario_factura():
    ...
    if conexion:
        try:
            ...
            consulta_actualizar_usuario = "UPDATE FACTURA SET DNI = :nuevo_dni WHERE id_factura = :id_factura"
            cursor.execute(consulta_actualizar_usuario, nuevo_dni=nuevo_dni_usuario, id_factura=id_factura)
            conexion.commit()
